...
impl Cache {
    ...
    pub async fn manifest_for_reference(&self, reference: impl AsRef<str>) -> Result<PathBuf> {
        ...
        if !p.is_dir() {
            fs::create_dir_all(&p)
                .await
                .context("cannot find directory for manifest")?;
        }
    ...
    pub async fn config_for_reference(&self, reference: impl AsRef<str>) -> Result<PathBuf> {
        let reference: Reference = reference
            .as_ref()
            .parse()
            .context("cannot parse reference")?;
