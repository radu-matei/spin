...
impl Store for InstanceScopedStore {
    ...
    async fn get_keys_async(
        ...
        tokio::sync::oneshot::Receiver<Result<(), crate::v3::Error>>,
    ) {
        let (mut inner_keys_rx, inner_err_rx) =
            self.inner.get_keys_async(max_result_bytes).await;
        let (keys_tx, keys_rx) = tokio::sync::mpsc::channel(4);
        let (err_tx, err_rx) = tokio::sync::oneshot::channel();
        let prefix = self.prefix.clone();

        // Stream keys from the underlying store, keeping only those belonging to
        // this instance and stripping the instance prefix before forwarding.
        tokio::spawn(async move {
            while let Some(key) = inner_keys_rx.recv().await {
                if let Some(unprefixed) = key.strip_prefix(&prefix) {
                    if keys_tx.send(unprefixed.to_string()).await.is_err() {
                        break;
                    }
                }
            }
            let res = inner_err_rx.await.unwrap_or(Ok(()));
