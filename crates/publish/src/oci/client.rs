...
impl Client {
    ...
    pub async fn push(&mut self, app: &Application, reference: impl AsRef<str>) -> Result<()> {
        ...
        for mut c in config.components {
        ...
        config.metadata.remove(&"origin".to_string());

        let oci_config = Config {
            data: serde_json::to_vec(&config)?,
    ...
    pub async fn pull(&mut self, reference: &str) -> Result<()> {
        ...
        let m = self
            .cache
            .manifest_for_reference(&reference.to_string())
            .await?;
        ...
        let c = self
            .cache
            .config_for_reference(&reference.to_string())
            .await?;
    ...
    pub fn auth(reference: &Reference) -> Result<RegistryAuth> {
    ...
    pub fn build_config(insecure: bool) -> ClientConfig {
    ...
    pub async fn data(file: &Path) -> Result<Vec<u8>> {
